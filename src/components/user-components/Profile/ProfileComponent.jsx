...
export default function profileComponent() {
  ...
  const handleSelection = (e) => {
    ...
    e.target.name == 'avatar' ? setIsAvatar(true) : setIsAvatar(false);
  };
  return (
    <>
          ...
          <div className='px-3 relative select-none'>
            <div className='left-0 right-0 mx-auto rounded-full h-32 w-32 bg-blue-50 '>
              <svg
                ...
                width={25}
                fill='black'
                className='absolute right-0 bottom-10 cursor-pointer text-red-50'
            ...
            {editDiv && (
              <div className='absolute min-w-max  h-auto min-h-full p-2  bg-white rounded-xl border shadow-xl'>
                <div className='options flex gap-2' onClick={handleSelection}>
                  <button name='avatar' className='rounded-md text-xs bg-slate-800 text-white p-2 '>
                    choose an avatar
                  </button>
                  <button name='upload' className='rounded-md text-xs bg-slate-800 text-white p-2'>
                    Upload from device
                  </button>
                </div>
                {isAvatar ? (
